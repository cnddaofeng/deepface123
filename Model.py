...
class SAEHDModel(ModelBase):
    ...
    def on_initialize_options(self):
        ...
        if self.is_first_run() or ask_override:
            ...
            # 修改：移除AdaBelief交互输入，硬编码为 False（关闭）
            self.options['adabelief'] = False  # 固定关闭 AdaBelief，以免用户误用；现在主推 Sophia-G
            ...
            # 修改：移除clipgrad交互输入，硬编码为 True（总是开启裁剪，以简化；用clipnorm控制值）
            self.options['clipgrad'] = True  # 固定开启梯度裁剪（防爆炸），值由clipnorm设置
        ...
        # 修复：Sophia代码移到方法内（使用self.options.get()）
        default_optimizer = self.options.get('optimizer', 'adabelief')  # 默认用 adabelief，如果未设置
        default_rho = self.options.get('rho', 0.04)
        default_weight_decay = self.options.get('weight_decay', 0.1)
        default_clipnorm = self.options.get('clipnorm', 1.0)  # 默认1.0（开启），合并clipgrad逻辑
        default_lr = self.options.get('lr', 5e-5)  # 原版默认学习率
        default_use_fp16 = self.options.get('use_fp16', False)
        default_batch_size = self.options.get('batch_size', 4)  # 从 .dat 脚本

        if self.is_first_run() or ask_override:  # 只在首次运行或覆盖模式下询问
            self.options['optimizer'] = io.input_str(
                '优化器类型 (optimizer)',
                default_optimizer,
                ['adabelief', 'sophia'],  # 选项列表，用户输入
                help_message='选择 sophia 以使用 Sophia-G 优化器。'
            )

            if self.options['optimizer'] == 'sophia':
                self.options['rho'] = io.input_number(
                    'Sophia rho 值',
                    default_rho,
                    add_info='0.01-0.1',
                    help_message='Sophia-G 二阶裁剪阈值。推荐 0.04。'
                )
                self.options['weight_decay'] = io.input_number(
                    '权重衰减 (weight_decay)',
                    default_weight_decay,
                    add_info='0.0-1.0',
                    help_message='Sophia-G 权重衰减。推荐 0.1。'
                )
                self.options['clipnorm'] = io.input_number(
                    '梯度裁剪 (clipnorm)',
                    default_clipnorm,
                    add_info='0.0-5.0',
                    help_message='防止梯度爆炸。推荐 1.0（0.0 表示关闭）。'
                )
            ...
            self.options['lr'] = io.input_number(
                ...
                default_lr,
                add_info='1e-6 - 1e-3',
                help_message='初始学习率。Sophia-G 推荐 2e-4。'
            )
            self.options['use_fp16'] = io.input_bool(
                '使用半精度 (use_fp16)',
                default_use_fp16,
                help_message='开启以加速 P106，但可能牺牲精度。'
            )
            self.options['batch_size'] = io.input_int(
                '批量大小 (batch_size)',
                default_batch_size,
                add_info='2-16',
                help_message='P106 6GB 推荐 4。'
            )
    ...
    def on_initialize(self):
        ...
        if self.is_training:
            ...
            clipnorm = 0.1 if bool(self.options['clipgrad']) else 0.0
            ...
            if 'df' in archi_type:
            	...
            	# 在on_initialize方法中，优化器创建部分修改为：
            # 1. 创建主优化器（使用PyTorch标准方式）
            self.src_dst_opt = self._create_sophia_optimizer(
                self.src_dst_trainable_weights,
                lr=lr,
            ...
            if float(self.options['true_face_power']) != 0.0 and 'df' in archi_type:
                self.D_code_opt = self._create_sophia_optimizer(
            ...
            if gan_power != 0.0:
                self.D_src_dst_opt = self._create_sophia_optimizer(
        ...
        for model, filename in io.progress_bar_generator(self.model_filename_list, 'Initializing models'):
            # --- 新增的安检门：如果没有 load_weights 方法（比如 SophiaG），直接跳过这套旧逻辑 ---
            ...
            # --- 新增安全判定：确保有 init_weights 方法才执行初始化 ---
            if do_init and hasattr(model, 'init_weights'):
                model.init_weights()
    ...
    def get_model_filename_list(self):
        ...
        # 必须大写 S，否则 ModelBase 不会执行它的覆写！
    def onSave(self):
        for model, filename in io.yield_bar(self.model_filename_list, desc="Saving models"):
    ...
    def train_one_step(self, warped_src, target_src, target_srcm, target_srcm_em, warped_dst, target_dst, target_dstm, target_dstm_em):
        ...
        if self.gan_power != 0.0:
            ...
            G_loss = G_loss + self.gan_power * (
                ...
                # === 核心修改：使用 update(grads) 模式 ===
        s_loss_val = float(src_loss_vec.mean().detach().cpu())
        d_loss_val = float(dst_loss_vec.mean().detach().cpu())
        
        # 1. 主优化器更新
        grads = torch.autograd.grad(G_loss, self.src_dst_trainable_weights, retain_graph=True)
        self.src_dst_opt.update(grads)

        # 2. TrueFace 更新 (如果开启)
        if hasattr(self, 'D_code_opt') and D_code_loss is not None:
            d_code_weights = list(self.code_discriminator.get_weights())
            d_code_grads = torch.autograd.grad(D_code_loss, d_code_weights, retain_graph=True)
            self.D_code_opt.update(d_code_grads)

        # 3. GAN 更新 (如果开启)
        if hasattr(self, 'D_src_dst_opt') and D_gan_loss is not None:
            d_gan_weights = list(self.D_src.get_weights())
            d_gan_grads = torch.autograd.grad(D_gan_loss, d_gan_weights)
            self.D_src_dst_opt.update(d_gan_grads)

        # 【直接返回结果，干掉原版所有废弃的 self.D_code_opt.step() 等代码】
        return s_loss_val, d_loss_val
    ...
    # ...（类前面的代码不变，比如 def get_MergerConfig(self): ...）

    def get_summary(self):
        """返回模型摘要表格字符串，用于 CMD 显示"""
        summary_lines = [
            "模型摘要 - SAEHD with Sophia-G",
            "-----------------------------",
            f"分辨率: {self.options.get('resolution', '未知')}",
            f"脸型: {self.options.get('face_type', '未知')}",
            f"架构: {self.options.get('archi', '未知')}",
            f"优化器: {self.options.get('optimizer', 'adabelief')}",
            f"学习率 (lr): {self.options.get('lr', 5e-5)}",
            f"批量大小 (batch_size): {self.options.get('batch_size', 4)}",
            f"半精度 (use_fp16): {self.options.get('use_fp16', False)}",
        ]

        # 添加 Sophia 专属参数（如果启用）
        if self.options.get('optimizer') == 'sophia':
            summary_lines.extend([
                "Sophia-G 参数:",
                f"  rho: {self.options.get('rho', 0.04)}",
                f"  权重衰减 (weight_decay): {self.options.get('weight_decay', 0.1)}",
                f"  梯度裁剪 (clipnorm): {self.options.get('clipnorm', 0.0)}",
            ])

        # 其他原选项（可扩展）
        summary_lines.extend([
            f"GAN 强度: {self.options.get('gan_power', 0.0)}",
            f"预训练: {self.options.get('pretrain', False)}",
            "-----------------------------",
        ])

        return "\n".join(summary_lines)  # 返回字符串，DFL 会打印它
