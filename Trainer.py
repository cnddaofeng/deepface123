...
import pickle  # 新增：导入pickle模块，用于保存.dat文件
        ...
        try:
            ...
            def model_save():
                if not debug and not is_reached_goal:
                    ...
                    model.save()  # 原调用
                    # 新增：强制额外保存 model_data_path（确保数据写入）
                    try:
                        model_data = {
                            'iter': model.iter,
                        ...
                        with open(model.model_data_path, 'wb') as f:
                            pickle.dump(model_data, f)  # 显式写入数据（iter、options等）
                        io.log_info("保存成功：模型已完整写入！文件大小约112MB")  # 日志确认
                    except Exception as e:
                        io.log_info(f"保存失败：{str(e)} 请检查路径权限或import pickle")  # 打印详细错误
            ...
            def model_backup():
                if not debug and not is_reached_goal:
                    ...
                    # 新增：检查备份文件大小（确保不为空）
                    try:
                        backup_path = model.autobackups_path / '01' / f'{model.get_model_name()}_data.dat'  # 假设第一个备份路径
                        if backup_path.exists() and backup_path.stat().st_size > 0:
                            io.log_info(f"备份成功：文件大小 {backup_path.stat().st_size / (1024*1024):.2f}MB")  # 确认大小
                        else:
                            io.log_info("备份失败：文件为空！请检查保存逻辑")
                    except Exception as e:
                        io.log_info(f"备份检查失败：{str(e)}")
            ...
            for i in itertools.count(0,1):
                ...
                # 新增：防抖变量（放在这里，循环每次重置）
                last_backup_time = 0  # 记录上次备份时间，防止重复
                ...
                while not s2c.empty():
                            ...
                            if op == 'save':
                                ...
                                if current_time - last_backup_time > 2:  # 增加到2秒间隔（防重复）
                                    model_backup()
                                    last_backup_time = current_time
                                    io.log_info("备份完成：只创建了一个完整备份！")  # 日志确认
                                else:
                                    io.log_info("备份请求太频繁，忽略重复！")  # 日志忽略
                            elif op == 'preview':
                                if is_reached_goal:
                                    model.pass_one_iter()
                                send_preview()
